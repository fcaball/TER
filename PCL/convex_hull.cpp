...
main (int argc, char** argv)
{
  ...
  vtk_file_indices = parse_file_extension_argument (argc, argv, ".ply");
  if (vtk_file_indices.size () != 1)
  {
    print_error ("Need one output PLY file to continue.\n");
    return (-1);
  }
...
#include <pcl/visualization/point_cloud_color_handlers.h>
  ...
  // Save the mesh
  io::savePLYFile (argv[vtk_file_indices[0]], mesh_out);

  pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("3D Viewer"));
  viewer->addPolygonMesh(mesh_out, "mesh", 0);
